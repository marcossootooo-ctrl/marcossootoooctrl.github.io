...
const App: React.FC = () => {
  ...
  const glowColors = useMemo(() => {
    if (screen === AppScreen.INTRO) return { c1: '#9333ea', c2: '#3b82f6', c3: '#fb923c' };
    if (screen === AppScreen.ONBOARDING) return { c1: '#3b82f6', c2: '#14b8a6', c3: '#9333ea' };
    if (screen === AppScreen.SELECTION) return { c1: '#14b8a6', c2: '#f43f5e', c3: '#eab308' };
    if (screen === AppScreen.SUMMARY) return { c1: '#ef4444', c2: '#ea580c', c3: '#f97316' };
    if (sport) return { c1: sport.color, c2: `${sport.color}cc`, c3: `${sport.color}99` };
    return { c1: '#9333ea', c2: '#3b82f6', c3: '#fb923c' };
  }, [screen, sport]);
    ...
    return (
      ...
      >
        <div className="mt-24 flex flex-col items-center text-center">
          <FadeIn delay={0.2}>
            <p className="text-zinc-500 font-medium mb-2 uppercase tracking-[0.2em] text-[10px]">Entrenamiento de Élite</p>
            <h1 className="text-7xl font-black tracking-tighter text-glow mb-8 drop-shadow-2xl">TrainUp</h1>
            <div className="text-sm opacity-60 space-y-1 font-medium">
              <p>Tu evolución comienza hoy.</p>
              <p>Desbloquea tu potencial levantando la pesa.</p>
            </div>
          </FadeIn>
        </div>
        <div className="flex flex-col items-center gap-6 mb-16 relative">
          <p className={`text-[11px] font-black uppercase tracking-widest transition-colors ${liftProgress > 85 ? 'text-orange-500' : 'text-zinc-500'}`}>
            {liftProgress > 0 ? (liftProgress > 85 ? '¡CASI LO TIENES!' : '¡ESO ES! SIGUE...') : 'ARRASTRA HACIA ARRIBA'}
          </p>
          <div className="relative w-full flex justify-center py-20" style={{ height: '350px' }}>
            <div className="absolute left-1/2 -translate-x-1/2 top-0 bottom-10 w-2 bg-white/5 rounded-full overflow-hidden">
               <div className="w-full bg-gradient-to-t from-white/10 to-white/60 rounded-full transition-all duration-75 shadow-[0_0_20px_rgba(255,255,255,0.4)]" style={{ height: `${liftProgress}%` }}></div>
            </div>
            <div onMouseDown={handleLiftStart} onTouchStart={handleLiftStart} className={`absolute bottom-10 cursor-grab active:cursor-grabbing transition-transform duration-75 ${isLifting ? 'scale-110' : 'hover:scale-105'}`} style={{ transform: `translateY(-${liftProgress * 3.2}px) ${isLifting ? `rotate(${Math.sin(liftProgress * 20) * (1 + liftProgress / 10)}deg)` : ''}` }}>
              <div className="relative flex items-center justify-center filter drop-shadow-[0_20px_40px_rgba(0,0,0,0.8)]">
                <div className="w-20 h-32 bg-[#1a1a1c] rounded-xl neu-convex border-l border-white/10 flex items-center justify-center"><div className="w-1 h-20 bg-black/40 rounded-full"></div></div>
                <div className="w-28 h-8 bg-zinc-800 neu-convex flex items-center justify-center relative overflow-hidden"><div className="absolute inset-0 bg-gradient-to-r from-transparent via-white/10 to-transparent animate-shimmer"></div></div>
                <div className="w-20 h-32 bg-[#1a1a1c] rounded-xl neu-convex border-r border-white/10 flex items-center justify-center"><div className="w-1 h-20 bg-black/40 rounded-full"></div></div>
              </div>
            </div>
          </div>
        </div>
  ...
  const renderSummary = () => {
    const mainGradient = "from-red-500 to-orange-600";
          ...
          {!summaryData ? (
            <FadeIn delay={0.2}>
              ...
              <button 
                ...
                className={`w-full mt-6 py-5 rounded-2xl bg-gradient-to-r ${mainGradient} text-white font-black uppercase tracking-widest text-xs shadow-2xl active:scale-95 flex items-center justify-center gap-3 transition-all`}
              >
                {isGeneratingSummary ? (
                  <>
            ...
            <div className="space-y-6 pb-12">
              {/* Gráfica de Calorías (Barra estilizada) */}
              <FadeIn delay={0.1}>
                <div className="neu-convex rounded-3xl p-6 border border-white/5 shadow-lg relative overflow-hidden">
                  ...
                  <div className="w-full h-4 bg-zinc-900 rounded-full overflow-hidden neu-concave">
                    <div 
                      className={`h-full bg-gradient-to-r ${mainGradient} shadow-[0_0_15px_rgba(239,68,68,0.5)]`}
                      style={{ width: `${Math.min((summaryData.calories / 1000) * 100, 100)}%`, transition: 'width 1.5s ease-out' }}
                    ></div>
                  </div>
                </div>
              </FadeIn>
              ...
              <div className="grid grid-cols-2 gap-4">
                <FadeIn delay={0.2}>
                  <div className="neu-convex rounded-3xl p-5 border border-white/5 flex flex-col items-center justify-center text-center">
                    ...
                    <span className="text-[10px] font-bold text-orange-500 mt-1">Líquidos (kg)</span>
                  </div>
                </FadeIn>
                <FadeIn delay={0.3}>
                  <div className="neu-convex rounded-3xl p-5 border border-white/5 flex flex-col items-center justify-center text-center">
                    ...
                    <div className="flex gap-1 mt-2">
                      {[...Array(10)].map((_, i) => (
                        <div key={i} className={`size-1 rounded-full ${i < summaryData.intensity ? 'bg-red-500' : 'bg-zinc-800'}`}></div>
                      ))}
                    </div>
                  </div>
                </FadeIn>
              ...
              {/* Gráfica de Fatiga (Pie Chart / Gauge) */}
              <FadeIn delay={0.4}>
              ...
              {/* Recovery Tip */}
              <FadeIn delay={0.5}>
                <div className="neu-convex rounded-3xl p-6 border border-orange-500/20 bg-orange-500/5 relative overflow-hidden">
        ...
        <main className={`flex-1 overflow-y-auto px-6 py-4 custom-scrollbar ${activeTab === DashboardTab.COACH ? 'pb-44' : 'pb-32'}`}>
          {activeTab === DashboardTab.PANEL && (
            <FadeIn>
              ...
              {/* Botón Finalizar Entreno Principal */}
              <div className="mb-8">
                <button 
                  onClick={() => setScreen(AppScreen.SUMMARY)}
                  className="w-full py-6 rounded-3xl bg-gradient-to-br from-red-500 to-orange-600 text-white shadow-[0_20px_40px_rgba(239,68,68,0.3)] active:scale-95 transition-all flex flex-col items-center justify-center gap-1 group"
                >
                  ...
                  <span className="text-[9px] opacity-70 font-bold">CALCULAR MÉTRICAS CON IA</span>
                </button>
              </div>
